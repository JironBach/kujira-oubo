 ...
 <head>
  ...
  <script>
		    ...
		    function addGroupInput(){
				    /*下記ソースだとグループの追加をするとデータが消える。L137-L157に変更
				    var last_counter = parseInt(document.getElementById("hidden_group").value);
					var	div_group_all = document.getElementById("div_group_all").innerHTML;
					...
					last_counter += 1; 
					div_group_all += "<div class='div_dynamic_input_text' id='div_group_" + last_counter + "'>" + 
					...
					addInnerHtml += "<div class='div_dynamic_input_text' id='div_group_" + last_counter + "'>" + 
									 ...
									 "<option value=''>選択してください</option>" + 
                                      groupOptions +
									 "</select><label style='font-size: 10px;'>あらかじめグループ（エリア）が登録されていればこちらの店舗をグループに分類することができます。(設定できるグループ数は5個までです。)</label><br> " + " <a onclick='delete_group(" + last_counter + ")' id='btn_del_group_" + last_counter + 
									 "' class='btn btn-rounded btn-danger' style='float:left; margin-top:5px; margin-right:5px; margin-bottom:5px;'>削除</a></div>";
                    addDiv.innerHTML = addInnerHtml;
			...
			function take_get_parameter(){
				if('setting_store' == '${param.from_which}'){
				var app_store_id   = '<%=storeObj.getId()%>';
				var app_store_name = '<%=storeObj.getName()%>';
				var	app_company_name = '<%=storeObj.getCompanyName()%>';
				var	app_company_furigana = '<%=storeObj.getCompanyFurigana()%>';
				var	app_store_manager = '<%=storeObj.getStoreManager()%>';
				var	app_group = '<%=storeObj.getStoreGroup()%>';
				var	app_work_loc_todoufuken = '<%=storeObj.getWorkLocTodoufuken()%>';
				var	app_work_loc_shikuchouson = '<%=storeObj.getWorkLocShikuchouson()%>';
				var	app_access_todofuken = '<%=storeObj.getAccessTodofuken()%>';
				var	app_access_train_company = '<%=storeObj.getAccessTrainCompany()%>';
				var	app_access_train_line = '<%=storeObj.getAccessTrainLine()%>';
				var	app_access_train_station = '<%=storeObj.getAccessTrainStation()%>';
				var	app_access_from_station = '<%=storeObj.getAccessFromStation()%>';
				var	app_access_by = '<%=storeObj.getAccessBy()%>';
				var	app_access_by_time = '<%=storeObj.getAccessByTime()%>';
				var app_access_comment = '<%=storeObj.getAccessComment()%>';
				var app_division = '<%=storeObj.getDivision()%>';
				var app_division_todofuken = '<%=storeObj.getDivisionTodofuken()%>';
				var app_division_shikuchouson = '<%=storeObj.getDivisionShikuchouson()%>';
				var app_division_house_number = '<%=storeObj.getDivisionHouseNumber()%>';
				var app_division_building_name = '<%=storeObj.getDivisionBuildingName()%>';
				var app_division_comment = '<%=storeObj.getDivisionComment()%>';
				var app_telephone_0 = '<%=storeObj.getTelephone0()%>';
				var app_telephone_1 = '<%=storeObj.getTelephone1()%>';
				var app_telephone_2 = '<%=storeObj.getTelephone2()%>';
				var app_telephone_comment = '<%=storeObj.getTelephoneComment()%>';
				var app_telephone_more_comment = '<%=storeObj.getTelephoneMoreComment()%>';
				var app_admin_comment = '<%=storeObj.getAdminComment()%>';
				var app_store_term = '<%=storeObj.getStoreTerm()%>';
				   }else{
			...
			/*	if (app_store_manager != "") {
					...
					for (var i = 0; i < store_manager.length; i++) {
						if (i == 0) {
							document.getElementById("txt_store_manager_0").value = store_manager[i];
						}
						else {
							div_store_manager_all += "<div class='div_dynamic_input_text' id='div_store_manager_" + i + "'>" + 
										  " <input name='txt_store_manager' class='normal_input_text' id='txt_store_manager_"+ i + "' value='" + store_manager[i] + "' /> <a onclick='delete_manager(" + i + ")' id='btn_del_store_manager_" 
				...
				if (app_store_manager != "") {
					...
					for (var i = 0; i < store_manager.length; i++) {
						if(store_manager[i] == -1 ){//デフォルト値を-1にしていたため、デフォルト値が''で行き渡ればことif文は不要に
						   store_manager[i] = '';
						}
				...
				/* 下記ソースだと戻るボタンでグループの設定が戻らないのでL331-L371のソースに変える
				//if (app_group != "") {
				{
				//	var group = new Array(),
				//   		group = app_group.split(",");
					...
					for (var i = 0; i < group.length; i++) {
							div_group_all += "<div class='div_dynamic_input_text' id='div_group_" + i + "'>" + 
										     ...
										     "<option value='-1'>選択をしてください</option>" + 
										     groupOptions +
				...
				if (app_access_todofuken != "" || app_access_train_company != "" || app_access_train_line != "" || app_access_train_line != "" || app_access_train_station != "" || app_access_from_station != "" || app_access_by != "" || app_access_by_time != "") {
					...
					for (var i = 0; i < access_todofuken.length; i++) {
						div_access_all += "<div id='div_access_" + i + "' class='div_access_box_0'><a style='margin-left:590px;'' class='a_nav_link' onclick='delete_access(" + i + ")' id='btn_del_access_" +  i + "'>削除</a>";    									
				...
				if (app_access_todofuken == ""){
					document.getElementById("hidden_access").value = 0;
				} else {
			...
			$(function() {
    			...
    			/*	var last_counter = parseInt(document.getElementById("hidden_store_manager").value);
					...
					div_store_manager_all += "<div class='div_dynamic_input_text' id='div_store_manager_" + last_counter + "'>" + 
										  " <select name='cmb_store_manager' class='normal_combobox' id='cmb_store_manager_"+ last_counter + "' >" + accountOptions + "</select><a onclick='delete_manager(" + last_counter + ")' id='btn_del_store_manager_" 
										  + last_counter + "' class='a_nav_link'>削除</a></div>";
			...
			function redirect_to_confirm_page() {
				...
				var input_store_id               = setParam("app_store_id","<%=storeObj.getId()%>");
                ...
                f.setAttribute('action',"/setting_store_upd_conf");
                f.appendChild(input_store_id);                
		...
		//	var prefSelect = document.getElementById('cmb_work_loc_todoufuken');
         ...
         <div class="box box-kujira">
          <div class="box-body">
           <div class="form-group">
       	    ...
       	    <h6 class="col-sm-6 col-xs-12" style="font-size:12px;"><%=storeObj.getId()%></h6>
           </div>
          </div>
       	  <div class="box-body" style="background-color: rgb(238, 120, 0, 0.2)">
       	   <div class="form-group">
       	   	...
       	   	<div class="col-lg-7 col-md-6 col-sm-12 col-xs-12">
       	   	 ...
       	   	 <div id="div_store_name_error" style="color: red;" class="error_msg"></div>
       	   	</div>
       	   </div>
       	  </div>
       	  <div class="box-body">
       	   <div class="form-group">
       	   	...
       	   	<div class="col-lg-7 col-md-6 col-sm-12 col-xs-12">
       	   	 ...
       	   	 <div id="div_company_name_error" style="color: red;" class="error_msg"></div>
       	   	</div>
       	   </div>
       	  </div>
       	  <div class="box-body" style="background-color: rgb(238, 120, 0, 0.2)">
       	  ...
       	  <div class="box-body" style="background-color: rgb(238, 120, 0, 0.2)">
       	   <div class="form-group">
       	   	...
       	   	<div class="col-md-7 col-xs-12">
       	   	 ...
       	   	 <div><a href="javascript:void(0)" class='btn btn-rounded btn-kujira' style='margin-top:5px;' onClick="addGroupInput()" id="btn_add_group">追加</a></div>
       	  ...
       	  <div class="box-body">
       	   ...
       	   <div id="div_work_loc_error" style="color: red;" class="error_msg"></div>
       	  </div>
       	  <div class="box-body" style="background-color: rgb(238, 120, 0, 0.2)">
       	   <div class="form-group">
       	  ...
       	  <div class="box-body">
       	   ...
       	   <div class="form-group">
       	    ...
       	    <div class="col-lg-5 col-md-10 col-sm-12 col-xs-12" style="margin-top:10px;">
       	     ...
       	     <div id="div_division_error" style="color: red;" class="error_msg"></div>
       	   	</div>
       	   </div>
       	  </div>
       	  <div class="box-body" style="background-color: rgb(238, 120, 0, 0.2)">
       	   ...
       	   <div id="div_telephone_error" style="color: red;" class="error_msg"></div>
       	  </div>
       	  <div class="box-body">
       	   <div class="form-group">
